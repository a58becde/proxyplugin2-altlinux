...
Если каталог со скриптами лежит в домашнем каталоге, смонтированном с `noexec`
(типично для доменных профилей), запускайте через интерпретатор — права на
исполнение в таком каталоге не действуют:

```bash
bash /путь/к/install.sh
```
